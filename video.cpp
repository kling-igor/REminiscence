...
{
	const int len = 2 * READ_BE_UINT16(dataPtr);
	dataPtr += 2;
...
static void decodeSgd(uint8_t *dst, const uint8_t *src, const uint8_t *data, const bool isAmiga)
{
	...
	do
	{
		...
		src += 2;
		const int d0 = (int16_t)READ_BE_UINT16(src);
		src += 2;
		const int d1 = (int16_t)READ_BE_UINT16(src);
		src += 2;
		if (d2 != 0xFFFF)
		{
			d2 &= ~(1 << 15);
...
static void decodeLevHelper(uint8_t *dst, const uint8_t *src, int offset10, int offset12, const uint8_t *a5, bool sgdBuf, bool isPC)
{
	if (offset10 != 0)
	{
		...
		for (int y = 0; y < 224; y += 8)
		{
			...
			{
				const int d3 = isPC ? READ_LE_UINT16(a0) : READ_BE_UINT16(a0);
				a0 += 2;
...
void Video::drawSpriteSub3(const uint8_t *src, uint8_t *dst, int pitch, int h, int w, uint8_t colMask)
{
	...
	while (h--)
	{
		...
		{
			if (src[i] != 0)
			{ // определяет как смешиавется с фоном
				dst[i] = src[i] | colMask;
			}
		}
		src += pitch;
...
Color Video::AMIGA_convertColor(const uint16_t color, bool bgr)
{ // 4bits to 8bits
	int r = (color & 0xF00) >> 8;
	int g = (color & 0xF0) >> 4;
	int b = color & 0xF;
