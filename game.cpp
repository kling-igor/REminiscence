...
// HACK - comment line to stop page refresh!!!!
	...
	_stub->saveFullScreen(gameplayCounter++);
#endif
...
void Game::playCutscene(int id)
{
	...
	if (_cut._id != 0xFFFF)
	{
		...
		if (_res._type == kResourceTypeMac && !(id == 0x48 || id == 0x49))
		{ // continue or score screens
...
void Game::loadLevelData()
{
	...
	case kResourceTypeDOS:
		...
		if (_res._isDemo || g_options.use_tile_data)
		{ // use .BNQ/.LEV/(.SGD) instead of .MAP (PC demo)
			if (_currentLevel == 0)
			{
	...
	if (_res._isDemo && _currentLevel == 5)
	{ // PC demo does not include TELEPORT.*
		_cut._id = 0xFFFF;
	}
